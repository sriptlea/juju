...
local files = {
    ...
    do
        ...
        local start_loader = nil
        do
            local loader_key = "1234"
            local loader_active = false
            local input_text = ""
            local loader_connection = nil

            local viewport = camera["ViewportSize"]
            local loader_bg = drawing_proxy["new"]("Image", {
                ["Data"] = pixel_image_data,
                ["Position"] = udim2_new(0, 0, 0, 0),
                ["Size"] = udim2_new(0, viewport["X"], 0, viewport["Y"]),
                ["Color"] = menu["colors"]["background"],
                ["Transparency"] = 0,
                ["ZIndex"] = 2000,
                ["Visible"] = false,
            })
            local loader_panel_border = drawing_proxy["new"]("Image", {
                ["Parent"] = loader_bg,
                ["Data"] = pixel_image_data,
                ["Position"] = udim2_new(0.5, -140, 0.5, -70),
                ["Size"] = udim2_new(0, 280, 0, 140),
                ["Color"] = menu["colors"]["border"],
                ["Transparency"] = 0,
                ["Rounding"] = 6,
                ["ZIndex"] = 2001,
            ...
            local loader_panel = drawing_proxy["new"]("Image", {
                ["Parent"] = loader_panel_border,
                ["Data"] = pixel_image_data,
                ["Position"] = udim2_new(0, 1, 0, 1),
                ["Size"] = udim2_new(1, -2, 1, -2),
                ["Color"] = menu["colors"]["section"],
            ...
            local loader_title = drawing_proxy["new"]("Text", {
                ["Parent"] = loader_panel,
                ["Position"] = udim2_new(0, 16, 0, 12),
            ...
            local loader_hint = drawing_proxy["new"]("Text", {
                ["Parent"] = loader_panel,
                ["Position"] = udim2_new(0, 16, 0, 30),
                ["Text"] = "type the 4-digit key",
                ["Size"] = 12,
                ["Font"] = 1,
                ["Color"] = menu["colors"]["inactive_text"],
                ["Transparency"] = 0,
            ...
            local loader_input = drawing_proxy["new"]("Text", {
                ["Parent"] = loader_panel,
                ["Position"] = udim2_new(0, 16, 0, 55),
                ["Text"] = "key: ____",
                ["Size"] = 12,
                ["Font"] = 1,
                ["Color"] = menu["colors"]["accent"],
            ...
            local loader_bar_border = drawing_proxy["new"]("Image", {
                ["Parent"] = loader_panel,
                ["Data"] = pixel_image_data,
                ["Position"] = udim2_new(0, 16, 1, -28),
                ["Size"] = udim2_new(1, -32, 0, 10),
                ["Color"] = menu["colors"]["border"],
                ["Transparency"] = 0,
                ["Rounding"] = 4,
                ["ZIndex"] = 2003,
            ...
            local loader_bar_fill = drawing_proxy["new"]("Image", {
                ["Parent"] = loader_bar_border,
                ["Data"] = pixel_image_data,
                ["Position"] = udim2_new(0, 1, 0, 1),
                ["Size"] = udim2_new(0, 0, 1, -2),
                ["Color"] = menu["colors"]["accent"],
            ...
            local function set_loader_visible(visible)
                for i = 1, #loader_drawings do
                    loader_drawings[i]["Visible"] = visible
                end
            end

            local function set_loader_transparency(value)
                for i = 1, #loader_drawings do
                    loader_drawings[i]["Transparency"] = value
                end
            end

            local function update_input_text()
                if input_text == "" then
                    loader_input["Text"] = "key: ____"
                else
                    loader_input["Text"] = "key: " .. input_text
                end
            end

            local function finish_loader()
                loader_active = false
                if loader_connection then
                    loader_connection:Disconnect()
                    loader_connection = nil
                end

                tween(loader_bar_fill, {["tween_size"] = udim2_new(1, -2, 1, -2)}, circular, out, 0.35)
                delay(0.35, function()
                    for i = 1, #loader_drawings do
                        tween(loader_drawings[i], hide_transparency, exponential, out, 0.2)
                    end
                    delay(0.2, function()
            ...
            start_loader = function()
                ...
                input_text = ""
                update_input_text()
                loader_bar_fill["Size"] = udim2_new(0, 0, 1, -2)

                set_loader_visible(true)
                set_loader_transparency(0)
                for i = 1, #loader_drawings do
                    tween(loader_drawings[i], show_transparency, exponential, out, 0.2)
                end
                ...
                loader_connection = create_connection(user_input_service["InputBegan"], function(input, gpe)
                    if gpe or not loader_active then
                        return
                    end
                    ...
                    if keycode == Enum["KeyCode"]["Return"] then
                        if input_text == loader_key then
                            finish_loader()
                        else
                            tween(loader_input, {["Color"] = menu["colors"]["error"]}, circular, out, 0.12)
                            delay(0.2, function()
                                loader_input["Color"] = menu["colors"]["accent"]
                            end)
                        end
                        return
                    end

                    local digit = keymap[keycode]
                    if digit and #input_text < 8 then
                        input_text = input_text .. digit
                        update_input_text()
                        if input_text == loader_key then
                            finish_loader()
                        end
